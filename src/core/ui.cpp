...
namespace VSTIR {
    ...
    void UI::sectionRenderSettings() {
        ...
        if (ImGui::TreeNodeEx("  ReSTIR GI Tuning", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::Text("  Temporal M Cap:");
            ImGui::SameLine();
            render_settings_changed |= ImGui::SliderInt("##tempmcap", &render_settings.temporal_m_cap, 1, 200);
            ImGui::Text("  Spatial M Cap:");
            ImGui::SameLine();
            render_settings_changed |= ImGui::SliderInt("##spatmcap", &render_settings.spatial_m_cap, 1, 1000);
            ImGui::Text("  ReSTIR GI Bounces:");
            ImGui::SameLine();
            render_settings_changed |= ImGui::SliderInt("##restirbounces", &render_settings.restir_bounces, 1, 4);
