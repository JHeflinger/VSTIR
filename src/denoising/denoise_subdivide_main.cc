...
{
    std::stringstream ss;
    std::stringstream ss_div;
    float bias = std::stof(argv[1]) * (argc >= 2);
    ss << bias;
    int divisions = std::stoi(argv[2]) * (argc >= 3);
    ss_div << divisions;
    ...
    else 
    {
        ...
        for (const auto& path: paths)
        {
            ...
            denoise_divide(img, stride, bias, divisions,false);
            ...
            write_img(img, stride,
                    "./denoised_subdivide_data/bias"+
                    ss.str()+"_" + 
                    "divisions"+ss_div.str()+"_" + 
